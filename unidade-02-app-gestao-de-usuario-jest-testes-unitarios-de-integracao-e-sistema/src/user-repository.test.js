...
describe('UserRepository', () =>{
    ...
    beforeEach(async ()=>{
        // Deleta TODOS os registros para no momento do teste describe('findOneByEmail) ele criar e retornar um único registro
    ...
    describe('findOneByEmail',  () =>{
        ...
        it('Deve lançar uma exceção para um usuário não existente ', async ()=> {
            await expect(userRepository.findOneByEmail('john@doe.com')).rejects.toThrow('Usuário com o email john@doe.com não existe!!')
        });
    ...
    describe('delete', () =>{
        it('Deve remover um usuário existente ', async ()=> {
            ...
            let resultado_esperado = { acknowledged: true, deletedCount: 1 }

            expect(result).toStrictEqual(resultado_esperado)
    ...
    describe('findAll', () =>{
        ...
        it('Deve retornar uma lista contendo usuários cadastrados', async ()=>{
            ...
            let arrayEmails = []
            results.map((result)=>{
                arrayEmails.push(result.email)
            })

            let arrayExpected = ['john@doe.com', 'john.segundo@doe.com']
